...
import moment from "moment";
import Errors from "../Errors";
import QRCode from "react-qr-code"; // Add this import - install: npm install react-qr-code
...
const UserProfile = () => {
  ...
  const enable2FA = async () => {
    ...
    try {
      ...
      if (typeof response.data === 'string') {
        // If backend returns base64 image or URL string
        setQrCodeUrl(response.data);
      } else if (response.data.qrCodeUrl) {
        // If backend returns object with qrCodeUrl and secretKey
  ...
  const verify2FA = async () => {
    ...
    if (code.length !== 6) {
      return toast.error("2FA code must be 6 digits");
    }
        ...
        <div className="xl:w-[70%] lg:w-[80%] sm:w-[90%] w-full sm:mx-auto sm:px-0 px-4 min-h-[500px] flex lg:flex-row flex-col gap-4">
          ...
          <div className="flex-1 flex flex-col shadow-lg shadow-gray-300 gap-2 px-4 py-6">
            <div className="space-y-1">
              ...
              <p className="text-slate-800 text-sm">
                Two Factor Authentication Add an additional layer of security
                to your account
              </p>
            </div>
            ...
            <div className="flex gap-2">
              ...
              {step === 2 && (
                <button
                  onClick={resetSetup}
                  className="bg-gray-500 text-white px-3 py-1 rounded-sm mt-2 hover:bg-gray-600"
                >
                  Cancel Setup
                </button>
              )}
                        ...
                        <div className="flex justify-center mb-4">
                          <div className="bg-white p-4 border border-gray-300 rounded-lg inline-block">
                            {qrCodeUrl.startsWith('data:image') ? (
                              ...
                              // If backend returns TOTP URL, render with QRCode component
                      ...
                      <div className="border-t pt-4">
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-2 justify-center">
                          <input
                            ...
                            maxLength={6}
                            required
                            className="border px-3 py-2 border-slate-300 rounded-md text-center text-lg font-mono w-32"
                            onChange={(e) => {
                          ...
                          />
                          <button
                            className="bg-btnColor text-white px-4 py-2 rounded-md hover:bg-opacity-90 disabled:opacity-50"
                            onClick={verify2FA}
                            disabled={twofaCodeLoader || code.length !== 6}
                          >
